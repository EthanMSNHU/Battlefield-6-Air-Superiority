/**
 * Airsup_v1
 * Baseline Air Sup ruleset with capture-point scoring and custom scoreboard stats.
 *
 * Overview:
 * - Implements the core match loop, objective ownership scoring, and player stat tracking.
 * - Uses a custom two-team scoreboard with Score, Kills, Deaths, and Captures columns.
 *
 * Version Focus:
 * - Establishes the scoring/stat foundation that all later HUD versions build on.
...
export async function OnGameModeStarted() { // main entry point when the custom game mode starts
    // v1 has no custom HUD yet; startup is only objectives, scoring, and scoreboard layout.
