/**
 * Airsup_v2
 * First HUD-enabled Air Sup version built on the v1 scoring and stat systems.
 *
 * Overview:
 * - Preserves the baseline match rules while introducing a per-player HUD.
 * - Creates HUD widgets for active players and late joiners without duplicating UI trees.
 *
 * Version Focus:
 * - Bridges the project from scoreboard-only gameplay into player-specific on-screen UI.
...
// HUD TRACKING
// Stores player object IDs that already have a HUD so createHUD remains one-time per player.
