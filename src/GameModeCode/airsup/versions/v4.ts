/**
 * Airsup_v4
 * Stabilization pass that moves HUD and scoreboard updates onto authoritative local score state.
 *
 * Overview:
 * - Introduces `gameState` as the local source of truth for team scores.
 * - Updates the scoreboard header and HUD from one consistent state model.
 *
 * Version Focus:
 * - Shifts the project from ad-hoc UI refreshes toward setter-based, mutate-in-place updates.
...
interface PlayerHud {
    // Friendly score text widget for this player's mirrored HUD.
    blueScoreName: string;
    // Enemy score text widget for this player's mirrored HUD.
    redScoreName: string;
}

// Per-player widget-name registry so runtime updates can mutate existing widgets in place.
