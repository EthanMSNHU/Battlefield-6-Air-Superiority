/**
 * Air_Radar
 * Air Sup variant that layers a lightweight aircraft radar on top of the baseline mode systems.
 *
 * Scope:
 * - Keeps baseline scoreboard, capture-point scoring, and team-switch behavior.
 * - Adds a left-side radar with center mark, heading tick, and friendly/enemy contact blips.
 * - Limits displayed contacts to a small fixed count for readability and runtime stability.
 *
 * Runtime Model:
 * 1. Create one radar widget set per player.
 * 2. During ongoing updates, compute player yaw and rotate world contacts into radar-local space.
 * 3. Project nearby friendlies/enemies into capped radar slots with separate colors and heading ticks.
 *
 * Notes:
 * - The radar favors simple container geometry over complex art assets for cheap runtime updates.
...
// Current player ID used to build the unique widget-name suffix for one radar set.
let radarOwnerId = 0;
// Prevent duplicate radar widget creation for the same player.
...
function checkIsJet(vehicle: any): boolean {
    // Restrict radar contacts to the intended jet list instead of all vehicles.
    try {
...
// The same origin also acts as the projection center used during blip placement.
const RADAR_OX = -620;
const RADAR_OY = 75;
...
function createRadarForPlayer(player: mod.Player) {
    // Build the static radar frame and a fixed pool of contact widgets once per player.
    try {
...
function destroyRadarWidgets() {
    // Best-effort cleanup for the widget set tied to the current owner suffix.
    try {
        const s = "" + radarOwnerId;
...
function updateRadar(eventPlayer: mod.Player) {
    // Transform world-space contacts into player-relative radar coordinates each update.
    try {
        ...
        // Resolve the local forward direction used to rotate contacts into radar space.
        let yawRad = 0;
