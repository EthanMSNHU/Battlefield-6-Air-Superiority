/**
 * TeamSwapper
 * Focused Air Sup variant used to prototype and isolate the team-switch workflow.
 *
 * Scope:
 * - Preserves the baseline capture-point scoring and scoreboard flow.
 * - Adds a per-player UI panel that opens from triple-tap interact input.
 * - Handles button routing, interact-point spawning, and forced undeploy after team change.
 *
 * Runtime Model:
 * 1. Match setup initializes objectives, scoreboard state, and existing players.
 * 2. Each player gets a persistent team-switch widget registry and world interact point.
 * 3. Interact edge detection converts repeated presses into a triple-tap action.
 * 4. UI button events apply the team change and then undeploy for a clean respawn flow.
 *
 * Notes:
 * - This file stays intentionally narrow in purpose so team-switch behavior can be tuned in isolation.
...
interface PlayerTeamSwitchUi {
    // Stored widget names and transient state for a single player's switch UI.
    hintTextName: string;
...
// Per-player registry so interact handlers and button events can target the correct panel widgets.
...
export async function OnGameModeStarted() {
    // Reset match-level timers and clear any stale player UI bookkeeping.
    team1ScoreTimer = 0;
    team2ScoreTimer = 0;
...
function initializeAllCurrentPlayers() {
    // Players already in the server at startup still need their team-switch UI created.
...
function createTeamSwitchUi(player: mod.Player) {
    // Build the panel once, then show/hide it later with visibility and input-mode toggles.
...
function processInteractTap(player: mod.Player) {
    // Fallback local tap counter used by interact-point events to mirror the triple-tap behavior.
    const id = mod.GetObjId(player);
    const ui = playerTeamSwitchUiMap.get(id);
    if (!ui) return;
...
function ensureTeamSwitchInteractPoint(player: mod.Player) {
    // Spawn a reusable interact point slightly in front of the deployed player.
...
async function forceUndeployPlayer(player: mod.Player): Promise<void> {
    // Double-call improves undeploy reliability across runtime timing edge cases.
    if (!player || !mod.IsPlayerValid(player)) return;
